...
def checks():
    ...
    return [
        ...
        ("stripe_test_key", stripe_key.startswith("sk_test_"), "sk_test_で始まるテスト秘密鍵"),
        ("stripe_price", os.getenv("STRIPE_PRICE_ID", "").startswith("price_"), "price_で始まる月額Price ID"),
        ("stripe_webhook", os.getenv("STRIPE_WEBHOOK_SECRET", "").startswith("whsec_"), "whsec_で始まる署名シークレット"),
